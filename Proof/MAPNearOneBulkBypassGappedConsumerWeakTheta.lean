...
namespace MAPNearOneBulkBypassGappedConsumerWeakTheta
...
private theorem eventually_relativeDistance_floor_log_le_weakGap_fixedTheta
    {theta c : ℝ} (htheta : theta < 1) (hc : 0 < c) :
    ∀ᶠ X : ℝ in atTop,
      relativeDistance ⌊Real.log X⌋₊ ≤
        c * Real.rpow (Real.log X) (-theta) := by
  have hsmallL : ∀ᶠ L : ℝ in atTop,
      Real.rpow L (theta - 1) ≤ c / 24 := by
    have ht := tendsto_rpow_neg_atTop
      (show 0 < (1 - theta) by linarith)
  ...
  have hfloorLower : Real.log X < (⌊Real.log X⌋₊ : ℝ) + 1 :=
    Nat.lt_floor_add_one (Real.log X)
  have hden : Real.log X ≤ (⌊Real.log X⌋₊ : ℝ) + 24 := by linarith
  have hinv : 1 / ((⌊Real.log X⌋₊ : ℝ) + 24) ≤
      1 / Real.log X := one_div_le_one_div_of_le hlogPos hden
  have hcoarse : relativeDistance ⌊Real.log X⌋₊ ≤ 24 / Real.log X := by
    calc
      relativeDistance ⌊Real.log X⌋₊ ≤
          24 / ((⌊Real.log X⌋₊ : ℝ) + 24) :=
  ...
  have hfactor : Real.rpow (Real.log X) (-1 : ℝ) =
      Real.rpow (Real.log X) (-theta) *
        Real.rpow (Real.log X) (theta - 1) := by
    calc
      Real.rpow (Real.log X) (-1 : ℝ) =
          Real.rpow (Real.log X) ((-theta) + (theta - 1)) := by
        congr 1 <;> ring
      _ = Real.rpow (Real.log X) (-theta) *
          Real.rpow (Real.log X) (theta - 1) :=
        Real.rpow_add hlogPos _ _
  have htarget : 24 / Real.log X ≤
      c * Real.rpow (Real.log X) (-theta) := by
    have hnonneg : 0 ≤ Real.rpow (Real.log X) (-theta) :=
...
theorem regularNear_logSaving_of_bulk_gappedCumulative_and_high_saving
    (hbulk : FixedPrimitiveBulkPolylogDensity)
    (hCollar : JutilaGappedSplitSelectedCumulativeEventually)
    (theta : ℝ) (hThetaLow : 1 / 4 ≤ theta) (hTheta : theta < 1)
    (hHighSaving : PrimitiveRegularHighSavingAt theta) :
    ∀ K A epsilon : ℝ,
  ...
  obtain ⟨cHigh, hcHigh, hgapHigh⟩ := hHighSaving K hK
  ...
  let c : ℝ := min (min cLow cHigh) cPrincipal
  ...
  have hc : 0 < c := by
    dsimp [c]
    exact lt_min (lt_min hcLow hcHigh) hcPrincipal
  have hc_le_low : c ≤ cLow := by
    dsimp [c]
    exact (min_le_left _ _).trans (min_le_left _ _)
  have hc_le_high : c ≤ cHigh := by
    dsimp [c]
    exact (min_le_left _ _).trans (min_le_right _ _)
  have hc_le_principal : c ≤ cPrincipal := by
    dsimp [c]
    exact (min_le_right _ _)
  have hpoly := polylog_absorption K u (by norm_num [u])
  have hmesh := eventually_relativeDistance_floor_log_le_weakGap_fixedTheta (theta := theta) (c := c) hTheta hc
  have hgapLow := weakGap_le_regularLowGap K hK.le
  have hprincipalWeak : ∀ᶠ X : ℝ in atTop,
      c * Real.rpow (Real.log X) (-theta) ≤ cPrincipal := by
    filter_upwards [eventually_gt_atTop (Real.exp 1)] with X hX
    have hXpos : 0 < X := (Real.exp_pos 1).trans hX
    have hlogOne : 1 < Real.log X := by
      rw [Real.lt_log_iff_exp_lt hXpos]
      exact hX
    have hpowLeOne : Real.rpow (Real.log X) (-theta) ≤ 1 := by
      calc
        Real.rpow (Real.log X) (-theta) ≤ Real.rpow (Real.log X) 0 :=
          Real.rpow_le_rpow_of_exponent_le hlogOne.le (by linarith)
        _ = 1 := Real.rpow_zero _
    calc
      c * Real.rpow (Real.log X) (-theta) ≤ c * 1 :=
        mul_le_mul_of_nonneg_left hpowLeOne hc.le
      _ = c := by ring
      _ ≤ cPrincipal := hc_le_principal
  have hcollarDecay := WeakVKNear.weakGap_nearFactor_beats_polylog
    theta c (2 * K + 1) A hTheta hc
      (by positivity) hA.le
  ...
  have hA5Absorb := logRpow_le_scaledWeakGapPower_fixedTheta
    (theta := theta) (P := P + 1)
    (a := tau epsilon * a5FiberGapBudget) (c := c)
    hTheta (by linarith)
  ...
  have hevent : ∀ᶠ X : ℝ in atTop,
    ...
    let L : ℕ := ⌊Real.log X⌋₊
    let omega : ℝ := c * Real.rpow (Real.log X) (-theta)
    ...
    have hgapAll : ∀ (q : ℕ) [NeZero q]
      ...
      · by_cases hpsi : psi = 1
        ...
        · have hregular' : ¬ (psi ^ 2 = 1 ∧ rho.im = 0) := by
          ...
          have hpowMono : Real.rpow (Real.log X) (-theta) ≤
          ...
          have homegaLow : omega ≤
            ...
            dsimp [omega]
            calc
              c * Real.rpow (Real.log X) (-theta) ≤
                  cLow * Real.rpow (Real.log X) (-theta) :=
      ...
      · have hheight' : 3 ≤ |rho.im| := le_of_not_gt hheight
        ...
        have homegaHigh : omega ≤
            cHigh * Real.rpow (Real.log X) (-theta) := by
          dsimp [omega]
...
/-- The weighted-zero source endpoint with the fixed-`theta` high-saving leaf.
This is the direct replacement boundary for the old `PrimitiveRegularHighGap`
consumer; no conversion back to the hard-coded `3/4` API is used. -/
theorem apWeightedZeroMassLogSaving_of_bulk_selectedP53_and_high_saving
  ...
  MAPAPExceptionalUnconditional.apWeightedZeroMassLogSaving_of_compact_regular
    hCompact (regularNear_logSaving_of_bulk_gappedCumulative_and_high_saving
      hbulk (primitiveRegularCumulativeCount_le_of_gappedSelectedP53 hP53 hPrincipalP53)
      theta hThetaLow hTheta hHighSaving)

end
end MAPNearOneBulkBypassGappedConsumerWeakTheta

#print axioms MAPNearOneBulkBypassGappedConsumerWeakTheta.regularNear_logSaving_of_bulk_gappedCumulative_and_high_saving
#print axioms MAPNearOneBulkBypassGappedConsumerWeakTheta.apWeightedZeroMassLogSaving_of_bulk_selectedP53_and_high_saving
