...
/-!
# MRT Proposition 5.1 and the Lemma 2.9/Cauchy route to Corollary 5.3

This module starts the internal certification of the published input isolated in
`MRTCorollary53SourceBridge`.  It does **not** assert either published theorem.
`MRTProposition51` and `MRTLemma29` are theorem-valued propositions, and every
deduction keeps inhabitants of them as explicit premises.

There is an exact source-level range discrepancy which must remain visible.
MRT Proposition 5.1 is printed with `1 ≤ H ≤ X/2`, whereas Corollary 5.3 is
printed with `1 ≤ H ≤ X` and its proof says only to apply Proposition 5.1 to
`f e(a·/q)`.  Consequently the direct stationary-phase/Cauchy argument below
certifies the half-range conclusion.  The full printed `MRTCorollary53` therefore
remains outside this module.  For MAP this causes no open endpoint branch:
`baseAperture ε X ≤ X/2` for `X ≥ 1`, and the final theorem connects the
half-range result directly to the existing `MAPFarSourceReduction`.
-/

namespace MAPMRTProposition51Source
...
/-! ## Literal finite Dirichlet polynomials and additive twist -/

/-- The finite form of `D[f](1/2+it)`.  For a finitely supported `f`, choosing
`N` past its support is literally the Dirichlet series in MRT Lemma 2.9. -/
def finiteCriticalPolynomial (N : ℕ) (f : ℕ → ℂ) (t : ℝ) : ℂ :=
  ∑ n ∈ Finset.Icc 1 N,
    f n * (Real.sqrt n : ℂ)⁻¹ * MixedMeanFrontend.mellinPhase n t

/-- The paper's coefficient twist `f e(a·/q)`. -/
def additiveTwist (q a : ℕ) (f : ℕ → ℂ) (n : ℕ) : ℂ :=
  f n * fourier (n : ℤ) (((a : ℝ) / q : ℝ) : UnitAddCircle)

theorem norm_additiveTwist (q a : ℕ) (f : ℕ → ℂ) (n : ℕ) :
    ‖additiveTwist q a f n‖ = ‖f n‖ := by
  simp [additiveTwist]

theorem additiveTwist_eq_zero_of_eq_zero
    {q a n : ℕ} {f : ℕ → ℂ} (h : f n = 0) :
    additiveTwist q a f n = 0 := by
  simp [additiveTwist, h]

/-- Multiplying the coefficients by `e(an/q)` translates the additive
frequency by `a/q`, with no sign reversal. -/
theorem exponentialSum_additiveTwist
    (X : ℝ) (q a : ℕ) (f : ℕ → ℂ) (theta : ℝ) :
    exponentialSum X (additiveTwist q a f) theta =
      exponentialSum X f ((a : ℝ) / q + theta) := by
  unfold exponentialSum additiveTwist
  apply Finset.sum_congr rfl
  intro n hn
  rw [show ((((a : ℝ) / q + theta : ℝ) : UnitAddCircle)) =
      (((a : ℝ) / q : ℝ) : UnitAddCircle) +
        ((theta : ℝ) : UnitAddCircle) by
      exact AddCircle.coe_add 1 ((a : ℝ) / q) theta]
  rw [fourier_apply, fourier_apply, fourier_apply]
  rw [zsmul_add, AddCircle.toCircle_add, Circle.coe_mul]
  ring

theorem continuous_norm_finiteCriticalPolynomial
    (N : ℕ) (f : ℕ → ℂ) :
    Continuous (fun t : ℝ ↦ ‖finiteCriticalPolynomial N f t‖) := by
  unfold finiteCriticalPolynomial MixedMeanFrontend.mellinPhase
  fun_prop

theorem continuous_norm_criticalDirichletPolynomial
    (X : ℝ) (q₀ q₁ : ℕ) (f : ℕ → ℂ)
    (chi : DirichletCharacter ℂ q₁) :
    Continuous (fun t : ℝ ↦
      ‖criticalDirichletPolynomial X q₀ q₁ f chi t‖) := by
  unfold criticalDirichletPolynomial MixedMeanFrontend.mellinPhase
  fun_prop

/-! ## Exact source statement of Proposition 5.1(i) -/

structure Proposition51Input where
  X : ℝ
  H : ℝ
  beta : ℝ
  eta : ℝ
...
def Proposition51Admissible (cBetaEta cEta : ℝ)
    (p : Proposition51Input) : Prop :=
  1 ≤ p.H ∧ p.H ≤ p.X / 2 ∧
  0 < p.eta ∧ p.eta ≤ 1 ∧
...
def proposition51Energy (p : Proposition51Input) : ℝ :=
  ∫ theta in (p.beta - 1 / p.H)..(p.beta + 1 / p.H),
    ‖exponentialSum p.X p.f theta‖ ^ 2

def untwistedWindow
    (X H : ℝ) (f : ℕ → ℂ) (beta t : ℝ) : ℝ :=
  ∫ t' in (t - |beta| * H)..(t + |beta| * H),
    ‖finiteCriticalPolynomial ⌊2 * X⌋₊ f t'‖

def untwistedComponentIntegral
    (X H : ℝ) (f : ℕ → ℂ) (beta eta : ℝ)
    (component : OuterComponent) : ℝ :=
  let endpoints := componentEndpoints X beta eta component
  ∫ t in endpoints.1..endpoints.2,
    untwistedWindow X H f beta t ^ 2

def proposition51I
    (X H : ℝ) (f : ℕ → ℂ) (beta eta : ℝ) : ℝ :=
  ∑ component : OuterComponent,
    untwistedComponentIntegral X H f beta eta component

/-- Proposition 5.1(i), with the constants hidden by
`|β| ≪ η ≪ 1` exposed in the same way as `MRTCorollary53`.

This is a proposition naming the published theorem.  It is not an axiom and
has no inhabitant in this module. -/
def MRTProposition51 (cBetaEta cEta : ℝ) : Prop :=
  ∃ C : ℝ, 0 < C ∧ ∀ (p : Proposition51Input)
    (hp : Proposition51Admissible cBetaEta cEta p),
    proposition51Energy p ≤ C *
      (1 / (|p.beta| ^ 2 * p.H ^ 2) *
          proposition51I p.X p.H p.f p.beta p.eta +
        ordinaryError p.X p.H p.f p.beta p.eta)

/-! ## Exact finite form of Lemma 2.9 -/

/-- MRT Lemma 2.9, equation (33), in its exact finite-polynomial form.  This is
equivalent to the paper's finitely-supported formulation by choosing `N` past
the support.  The sum is over all positive ordered factorizations and all
Dirichlet characters, including the principal character. -/
def MRTLemma29 : Prop :=
  ∀ (N q a : ℕ) (f : ℕ → ℂ) (t : ℝ),
    1 ≤ q → a.Coprime q →
...
theorem lemma29_specialize_to_source
    (hL29 : MRTLemma29) {X : ℝ} {q a : ℕ} {f : ℕ → ℂ} {t : ℝ}
  ...
  simpa [criticalDirichletPolynomial] using
    hL29 ⌊2 * X⌋₊ q a f t hq haq

/-! ## Arithmetic cardinality and source-region geometry -/

theorem factorization_mem_divisors
    {q : ℕ} (hq : 1 ≤ q) {z : ℕ × ℕ}
    (hz : z ∈ modulusFactorizations q) : z.1 ∈ q.divisors := by
  rw [Nat.mem_divisors]
  constructor
  · refine ⟨z.2, ?_⟩
    simpa [modulusFactorizations] using (Finset.mem_filter.mp hz).2.symm
  · omega

theorem card_modulusFactorizations_le_divisorCount
    {q : ℕ} (hq : 1 ≤ q) :
    (modulusFactorizations q).card ≤ divisorCount q := by
  let e : {z // z ∈ modulusFactorizations q} → {d // d ∈ q.divisors} :=
    fun z ↦ ⟨z.1.1, factorization_mem_divisors hq z.2⟩
  have he : Function.Injective e := by
    intro z w hzw
    apply Subtype.ext
    apply Prod.ext
    · exact congrArg Subtype.val hzw
    · have hfirst : z.1.1 = w.1.1 := congrArg Subtype.val hzw
      have hzmem := (Finset.mem_filter.mp z.2).2
      have hwmem := (Finset.mem_filter.mp w.2).2
      have hzpos : 0 < z.1.1 := by
        have := (Finset.mem_product.mp (Finset.mem_filter.mp z.2).1).1
        exact (Finset.mem_Icc.mp this).1
      apply Nat.eq_of_mul_eq_mul_left hzpos
      calc
        z.1.1 * z.1.2 = q := hzmem
        _ = w.1.1 * w.1.2 := hwmem.symm
        _ = z.1.1 * w.1.2 := by rw [hfirst]
  simpa [divisorCount] using Fintype.card_le_of_injective e he

theorem componentEndpoints_mono
    {X beta eta : ℝ} (hX : 0 ≤ X) (heta : 0 < eta)
    (hetaOne : eta ≤ 1) (component : OuterComponent) :
    (componentEndpoints X beta eta component).1 ≤
      (componentEndpoints X beta eta component).2 := by
  have hnonneg : 0 ≤ |beta| * X := mul_nonneg (abs_nonneg _) hX
  have houter : outerLower X beta eta ≤ outerUpper X beta eta := by
    unfold outerLower outerUpper
    have hetaSq : eta ^ 2 ≤ 1 := by nlinarith
    apply (le_div_iff₀ heta).2
    nlinarith
  cases component <;> simpa [componentEndpoints] using houter

theorem characterWindow_nonneg
    {X H beta t : ℝ} {q₀ q₁ : ℕ} {f : ℕ → ℂ}
    (hU : 0 ≤ |beta| * H) :
    0 ≤ characterWindow X q₀ q₁ f beta H t := by
...
theorem sourceI_nonneg
    {X H beta eta : ℝ} {q₀ q₁ : ℕ} {f : ℕ → ℂ}
    (hX : 0 ≤ X) (hH : 0 ≤ H) (heta : 0 < eta) (hetaOne : eta ≤ 1) :
    0 ≤ sourceI X H q₀ q₁ f beta eta := by
  unfold sourceI componentIntegral
  apply Finset.sum_nonneg
...
/-! ## Lemma 2.9 integrated over the stationary window -/

theorem continuous_untwistedWindow
    (X H : ℝ) (f : ℕ → ℂ) (beta : ℝ) :
    Continuous (untwistedWindow X H f beta) := by
  unfold untwistedWindow
  exact continuous_movingWindowIntegral
    (continuous_norm_finiteCriticalPolynomial ⌊2 * X⌋₊ f) (|beta| * H)

theorem continuous_characterWindow
    (X H : ℝ) (q₀ q₁ : ℕ) (f : ℕ → ℂ) (beta : ℝ) :
    Continuous (characterWindow X q₀ q₁ f beta H) := by
  unfold characterWindow
  apply continuous_finset_sum
  intro chi hchi
  exact continuous_movingWindowIntegral
    (continuous_norm_criticalDirichletPolynomial X q₀ q₁ f chi)
    (|beta| * H)

theorem untwistedWindow_nonneg
    {X H beta t : ℝ} {f : ℕ → ℂ} (hH : 0 ≤ H) :
    0 ≤ untwistedWindow X H f beta t := by
  unfold untwistedWindow
  apply intervalIntegral.integral_nonneg (by
...
theorem lemma29_window_bound
    (hL29 : MRTLemma29) {X H beta t : ℝ} {q a : ℕ} {f : ℕ → ℂ}
  ...
  have hpoint := fun t' ↦
    lemma29_specialize_to_source hL29 (X := X) (q := q) (a := a)
      (f := f) (t := t') hq haq
...
theorem lemma29_window_sq_bound
  ...
  have hwindow : W ≤ c * ∑ z ∈ modulusFactorizations q, B z :=
    lemma29_window_bound hL29 hH hq haq
...
theorem lemma29_componentIntegral_bound
    (hL29 : MRTLemma29) {X H beta eta : ℝ} {q a : ℕ} {f : ℕ → ℂ}
    (hX : 0 ≤ X) (hH : 0 ≤ H) (heta : 0 < eta) (hetaOne : eta ≤ 1)
    (hq : 1 ≤ q) (haq : a.Coprime q) (component : OuterComponent) :
  ...
  calc
    (∫ t in e.1..e.2,
      ...
      · exact hright.intervalIntegrable _ _
      · intro t ht
        exact lemma29_window_sq_bound hL29 hH hq haq
    _ = lemma29CauchyCoefficient q *
        ∑ z ∈ modulusFactorizations q,
          (∫ t in e.1..e.2,
            characterWindow X z.1 z.2 f beta H t ^ 2) := by
...
theorem lemma29_proposition51I_bound
  ...
  calc
    (∑ component : OuterComponent,
      ...
      intro component hcomponent
      exact lemma29_componentIntegral_bound hL29 hX hH heta hetaOne hq haq component
    _ = lemma29CauchyCoefficient q *
        ∑ z ∈ modulusFactorizations q,
          ∑ component : OuterComponent,
            componentIntegral X H z.1 z.2 f beta eta component := by
...
theorem lemma29_proposition51I_le_factorizationSup
  ...
  have hfirst := lemma29_proposition51I_bound hL29
    ...
    (q := q) (a := a) (f := f) hX hH heta hetaOne hq haq
...
theorem proposition51_main_le_stationaryMain
    (hL29 : MRTLemma29) {p : Corollary53Input}
    (hp : Corollary53Admissible cBetaEta cEta p) :
    1 / (|p.beta| ^ 2 * p.H ^ 2) *
  ...
  have hI := lemma29_proposition51I_le_factorizationSup hL29
    ...
    (q := p.q) (a := p.a) (f := p.f)
    hX (by linarith) heta hetaOne hq haq
...
/-- Direct MAP promotion from the two published MRT source inputs and the
existing source reduction.  All three premises remain visible. -/
theorem canonicalFarAnnulusEstimate_of_proposition51_lemma29_source
  ...
  canonicalFarAnnulusEstimate_of_halfRange_source
    (corollary53_halfRange_of_proposition51_lemma29 hP51 hL29) hsource

end

end MAPMRTProposition51Source

#print axioms MAPMRTProposition51Source.exponentialSum_additiveTwist
#print axioms MAPMRTProposition51Source.card_modulusFactorizations_le_divisorCount
#print axioms MAPMRTProposition51Source.lemma29_window_bound
#print axioms MAPMRTProposition51Source.lemma29CauchyCoefficient_eq
#print axioms MAPMRTProposition51Source.lemma29_proposition51I_le_factorizationSup
#print axioms MAPMRTProposition51Source.corollary53_halfRange_of_proposition51_lemma29
#print axioms MAPMRTProposition51Source.baseAperture_le_half_of_one_le
#print axioms MAPMRTProposition51Source.canonicalFarAnnulusEstimate_of_proposition51_lemma29_source
