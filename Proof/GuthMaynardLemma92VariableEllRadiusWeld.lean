import GuthMaynardSourceBumpScaling
...
/-!
# Variable-radius selected-range Lemma 9.2 endpoint

The affine smoothing radius is `B=T^δ`, while the independent ell-detector radius is `R`.  This is the source-faithful interface needed before taking `R=T^κ`: the detector Fourier constants are the certified linear-in-R constants from `GuthMaynardSourceBumpScaling`.
-/
...
theorem sigmaIIFinite_selectedPositiveDyadic_variableEllRadius_le
    ...
    (q : ℕ) {R Ctail : ℝ} (hR : 0 < R) (hRone : 1 ≤ R)
  ...
  have htail : ∀ m2 ∈ mRange, ∀ m2' ∈ mRange,
    ...
    exact integrable_sigmaIIZPairTailKernel_of_budget
      (M3 := (M : ℝ)) (fun x => sourceBump R hR x) f
      hf.integrable (sourceBump_complex_hasCompactSupport R hR)
      (sourceBump_complex_contDiff R hR) q hKdec hMreal hTpos
...
#print axioms GuthMaynardJIteration.sigmaIIFinite_selectedPositiveDyadic_variableEllRadius_le
