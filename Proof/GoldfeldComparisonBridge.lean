import SiegelExceptionalAbsorption
...
/-- The quantitative output of Goldfeld's smoothed contour comparison in the
proof of Koukoulopoulos Theorem 12.9 (printed pp. 125--127).  The two
characters are packaged across varying primitive levels; `a ≠ exceptional`
is the exact source condition that they are different. -/
def GoldfeldComparisonEstimate : Prop :=
  ∃ A C : ℝ, 0 < A ∧ 0 < C ∧
      ...
      ∀ betaExceptional : ℝ,
        ...
        exceptional.LFunction betaExceptional = 0 →
          C * Real.rpow (a.level : ℝ)
                (-A * (1 - betaExceptional)) /
              (Real.log (2 * (a.level : ℝ))) ^ 3 ≤
            ‖a.LFunction 1‖

/-- A positive power absorbs the logarithmic cube in Goldfeld's comparison,
with a completely explicit constant. -/
theorem log_two_mul_cube_le_const_mul_rpow
    {theta x : ℝ} (htheta : 0 < theta) (hx : 1 ≤ x) :
    (Real.log (2 * x)) ^ 3 ≤
      (Real.rpow 2 theta / (theta / 3) ^ 3) * Real.rpow x theta := by
  let r : ℝ := theta / 3
  ...
  have hcube := pow_le_pow_left₀ hlog0 hlogBound 3
  calc
    (Real.log (2 * x)) ^ 3 ≤ (Real.rpow (2 * x) r / r) ^ 3 := hcube
    _ = (Real.rpow 2 theta / (theta / 3) ^ 3) * Real.rpow x theta := by
      ...
      dsimp [r]
      rw [div_pow]
      have hpow2 : (Real.rpow 2 (theta / 3)) ^ 3 = Real.rpow 2 theta := by
        calc
          (Real.rpow 2 (theta / 3)) ^ 3 =
              Real.rpow (Real.rpow 2 (theta / 3)) (3 : ℝ) :=
            (Real.rpow_natCast _ 3).symm
          _ = Real.rpow 2 ((theta / 3) * 3) :=
            (Real.rpow_mul htwo (theta / 3) 3).symm
          _ = Real.rpow 2 theta := by congr 1 <;> ring
      have hpowx : (Real.rpow x (theta / 3)) ^ 3 = Real.rpow x theta := by
        calc
          (Real.rpow x (theta / 3)) ^ 3 =
              Real.rpow (Real.rpow x (theta / 3)) (3 : ℝ) :=
            (Real.rpow_natCast _ 3).symm
          _ = Real.rpow x ((theta / 3) * 3) :=
            (Real.rpow_mul hx0 (theta / 3) 3).symm
          _ = Real.rpow x theta := by congr 1 <;> ring
      rw [mul_pow]
      change (Real.rpow 2 (theta / 3)) ^ 3 *
          (Real.rpow x (theta / 3)) ^ 3 / (theta / 3) ^ 3 = _
      rw [hpow2, hpowx]
      change Real.rpow 2 theta * Real.rpow x theta / (theta / 3) ^ 3 =
        (Real.rpow 2 theta / (theta / 3) ^ 3) * Real.rpow x theta
      ring
...
theorem GoldfeldComparisonEstimate.lvalue_power_lower
    (hGoldfeld : GoldfeldComparisonEstimate)
    {nu : ℝ} (hnu : 0 < nu) :
    ∃ A C : ℝ, 0 < A ∧ 0 < C ∧
      ∀ (exceptional a : PrimitiveRealCharacter),
        exceptional.level ≤ a.level → a ≠ exceptional →
  ...
  let D : ℝ := Real.rpow 2 nu / (nu / 3) ^ 3
  have hD : 0 < D := div_pos (Real.rpow_pos_of_pos (by norm_num) nu)
    (pow_pos (div_pos hnu (by norm_num)) 3)
  let C : ℝ := C0 / D
  ...
  have hlogCube := log_two_mul_cube_le_const_mul_rpow hnu hqOne
  change (Real.log (2 * (a.level : ℝ))) ^ 3 ≤
  ...
  have hinv :
      (D * Real.rpow (a.level : ℝ) nu)⁻¹ ≤
        ((Real.log (2 * (a.level : ℝ))) ^ 3)⁻¹ := by
    exact (inv_le_inv₀
      (mul_pos hD (Real.rpow_pos_of_pos hqPos nu))
      (pow_pos hlogPos 3)).2 hlogCube
  have hmerge :
  ...
  calc
    ...
    _ = C0 * Real.rpow (a.level : ℝ) (-A * (1 - beta)) /
          (Real.log (2 * (a.level : ℝ))) ^ 3 := by rw [div_eq_mul_inv]
...
theorem GoldfeldComparisonEstimate.exists_allButOne_nearOne_or_lvalue
    (hGoldfeld : GoldfeldComparisonEstimate)
  ...
  rcases hGoldfeld.lvalue_power_lower (show 0 < epsilon / 4 by positivity) with
    ⟨A, C, hA, hC, hlower⟩
  let delta : ℝ := min (1 / 2) (epsilon / (4 * A))
  have hdelta : 0 < delta := lt_min (by norm_num) (by positivity)
  have hdeltaHalf : delta ≤ 1 / 2 := min_le_left _ _
  have hAdelta : A * delta ≤ epsilon / 4 := by
    have hd := min_le_right (1 / 2 : ℝ) (epsilon / (4 * A))
    calc
  ...
  · let qExceptional : ℕ := Nat.find hbad
    ...
    have hbetaHalf : 1 / 2 ≤ betaExceptional := by
      linarith
...
theorem GoldfeldComparisonEstimate.to_allButOneZeroFree
    (hGoldfeld : GoldfeldComparisonEstimate) :
...
theorem GoldfeldComparisonEstimate.to_publishedSiegelRealZeroFreeRegion
    (hGoldfeld : GoldfeldComparisonEstimate) :
...
#print axioms MAPGoldfeldSiegel.log_two_mul_cube_le_const_mul_rpow
#print axioms MAPGoldfeldSiegel.GoldfeldComparisonEstimate.lvalue_power_lower
#print axioms MAPGoldfeldSiegel.GoldfeldComparisonEstimate.exists_allButOne_nearOne_or_lvalue
#print axioms MAPGoldfeldSiegel.GoldfeldComparisonEstimate.to_allButOneZeroFree
#print axioms MAPGoldfeldSiegel.GoldfeldComparisonEstimate.to_publishedSiegelRealZeroFreeRegion
