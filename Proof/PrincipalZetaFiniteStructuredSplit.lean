...
namespace MAPPrincipalZetaFiniteStructuredSplit
...
/-- Finite conductor-one A.4/A.5 weld.  The Type-I coefficient provenance is
literal and the Type-II contribution is controlled only through its own
shifted fourth moment.  The target is distinct principal-zero support; the
separate principal crowding theorem restores multiplicity downstream. -/
...
end MAPPrincipalZetaFiniteStructuredSplit
