...
/-! # Uniform logarithmic budget for the three Type-II losses -/

namespace MRTProposition61TypeIIPolylogBudgetV3
...
theorem eventually_four_typeII_log_terms_le_thirtieth
      ...
      ∃ Cc₀ : ℕ, ∀ Cc : ℕ, Cc₀ ≤ Cc →
        ∀ᶠ X : ℝ in atTop,
          C * (Real.rpow (Real.log X) (P + 9 * (B : ℝ) / 8) *
            ...
            Real.rpow (Real.log X) (P + (B : ℝ) / 8 - Cc) +
            Real.rpow (Real.log X) (P + (B : ℝ) / 8) *
              Real.rpow X (-(1 / 4 : ℝ))) ≤
  ...
  obtain ⟨Cc₀, hCc₀⟩ := exists_nat_gt (P + (B : ℝ) / 8 + A + 1)
  refine ⟨Cc₀, ?_⟩
  intro Cc hCc
  have hCc' : (Cc₀ : ℝ) ≤ Cc := by exact_mod_cast hCc
  have hCcexp : P + (B : ℝ) / 8 - Cc < -A := by linarith
  ...
  have h3 := eventually_const_log_power_le_decay
    (120 * C) (P + (B : ℝ) / 8 - Cc) A hCcexp
  have h4 := eventually_const_polylog_mul_neg_rpow_le_log_decay
    (120 * C) (P + (B : ℝ) / 8) A (1 / 4) (by positivity) (by norm_num)
...
theorem typeII_scalar_envelope_le_four_log_terms
    ...
    C * Real.rpow (Real.log X) P *
      (Real.rpow Q (1 / 8 : ℝ) *
          (Q * Real.rpow X (-delta) + 1 / Real.sqrt Q + 1 / U) +
    ...
    _ ≤ C * Real.rpow (Real.log X) P *
        (Real.rpow ((Real.log X) ^ B) (1 / 8 : ℝ) *
            ((Real.log X) ^ B * Real.rpow X (-delta) +
              1 / Real.sqrt ((Real.log X) ^ B) + 1 / (Real.log X) ^ Cc) +
          Real.rpow ((Real.log X) ^ B) (1 / 8 : ℝ) *
            Real.rpow X (-(1 / 4 : ℝ))) := by
      gcongr
    _ = _ := by
      rw [hQ (1 / 8), hsqrt, hQbase, hCcbase]
...
theorem eventually_typeII_scalar_envelope_le_thirtieth
    (C P A delta : ℝ) (hC : 0 ≤ C) (hdelta : 0 < delta) :
    ∃ B₀ : ℕ, ∀ B : ℕ, B₀ ≤ B →
      ∃ Cc₀ : ℕ, ∀ Cc : ℕ, Cc₀ ≤ Cc →
        ∀ᶠ X : ℝ in atTop, ∀ U : ℝ, (Real.log X) ^ Cc ≤ U →
          ...
          C * Real.rpow (Real.log X) P *
            (Real.rpow Q (1 / 8 : ℝ) *
              ...
              Real.rpow Q (1 / 8 : ℝ) * Real.rpow X (-(1 / 4 : ℝ))) ≤
            Real.rpow (Real.log X) (-A) / 30 := by
  obtain ⟨B₀, hB₀⟩ := eventually_four_typeII_log_terms_le_thirtieth C P A delta hC hdelta
  refine ⟨B₀, ?_⟩
  intro B hB
  ...
  intro U hU
  exact (typeII_scalar_envelope_le_four_log_terms B Cc hX hC hU).trans hbudget

end
end MRTProposition61TypeIIPolylogBudgetV3

#print axioms MRTProposition61TypeIIPolylogBudgetV3.eventually_four_typeII_log_terms_le_thirtieth

#print axioms MRTProposition61TypeIIPolylogBudgetV3.eventually_typeII_scalar_envelope_le_thirtieth
