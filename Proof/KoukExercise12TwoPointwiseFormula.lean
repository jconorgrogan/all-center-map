...
namespace MAPKoukExercise12TwoPointwiseFormula
...
theorem exists_norm_primitivePrefix_sub_characterMain_le_zeroSum_add_contour
    {q : ℕ} [NeZero q] (chi : DirichletCharacter ℂ q)
  ...
  have hd : 0 < d := by
    dsimp [d]
    exact exerciseContourClearance_pos chi H
  ...
  have hlogBound (v r : ℝ) (hv : |v| ≤ H + 1)
    ...
    have hbase := norm_logDeriv_LFunction_le_of_wide_clearance
      chi hprim hchi hr0 hr4 hd hdist
  ...
  have hlogTop : ∀ r ∈ Set.Icc sigma c,
    ...
    exact hlogBound T r hTabs (hsigma.1.trans_le hr.1)
      (hr.2.trans_lt hc4) (hdistTop r hr)
  have hlogBottom : ∀ r ∈ Set.Icc sigma c,
      ‖logDeriv (DirichletCharacter.LFunction chi)
        ((r : ℂ) - Complex.I * T)‖ ≤ R := by
    intro r hr
    have hbase := hlogBound (-T) r hnegTabs (hsigma.1.trans_le hr.1)
      (hr.2.trans_lt hc4) (by
        intro rho hrho
...
#print axioms MAPKoukExercise12TwoPointwiseFormula.exists_norm_primitivePrefix_sub_characterMain_le_zeroSum_add_contour

end

end MAPKoukExercise12TwoPointwiseFormula
