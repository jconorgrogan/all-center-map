...
\usepackage{xurl}
\usepackage[colorlinks=true,linkcolor=MidnightBlue,citecolor=MidnightBlue,urlcolor=BrickRed,breaklinks=true]{hyperref}
...
\begin{abstract}
Write \(S_X\) for the von Mangoldt exponential sum on \((X,2X]\):
\[
 S_X(\alpha)=\sum_{X<n\le 2X}\Lambda(n)\,e^{2\pi i n\alpha}.
\]
For every \(A,\eps>0\), after removing narrow major arcs around rationals
with logarithmically bounded denominators, we prove
\[
 \sup_{|J|=1/H}\int_{J\setminus\mathfrak M}|S_X(\alpha)|^2\,d\alpha
 \ll_{A,\eps}X(\log X)^{-A}
 \qquad\text{for all }H\ge X^{2/15+\eps}.
\]
Consequently, in every translated shift interval of this length the
Hardy--Littlewood prime-pair formula holds in mean square and for all but
\(O\bigl(H(\log X)^{-A}\bigr)\) individual shifts, and the second moment of
\(R_X\) matches its singular-series prediction.  The same estimate yields
the weighted Goldbach formula for almost every target in \([X,X+H]\),
together with a balanced form whose summands lie in \((X,2X]\).  This
replaces the Matom\"aki--Radziwi\l\l--Tao exponent \(8/33\) by \(2/15\).
\end{abstract}
...
\begin{align*}
 \M(P,Q)
 &=\bigcup_{q\le Q}\ \bigcup_{\substack{a\bmod q\\(a,q)=1}}
...
The estimate~(1.1) is ready for Palomar submission.  The Lean theorem is
\begin{center}
\texttt{AllCenterMAP.map\_two\_fifteenths}\\[3pt]
{\small\url{https://github.com/jconorgrogan/prime-minor-arcs-2-15}}
\end{center}
\end{remark}
...
and is ready for Palomar submission, with
...
\input{polylog_zero_density_lemma}
