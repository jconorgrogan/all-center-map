import JutilaP53SharpPairSectors
...
namespace MAPJutilaP53SharpIntegratedPairSectorAdapter
...
theorem integratedCorrelationEstimateAt_of_sharpPairSectorEnvelopes
  ...
  have hPoint : ∀ xi ∈ Set.Icc ((1 - epsilon) * Real.log z1) (Real.log z1),
    ...
    have hxi0 : 0 ≤ xi := by
      have := Real.log_nonneg hz1
      have : 0 ≤ (1 - epsilon) * Real.log z1 := mul_nonneg (by linarith) this
      exact this.trans hxi.1
    have hpair := MAPJutilaP53SharpPairSectors.jutilaP53CorrelationEnergy_le_sharpPairSectors rows hS hSq
      hepsHi (Real.one_le_exp hxi0) (Real.exp_lt_exp.mpr hMNlog)
      hcop hrows hfiberSep hfiberCard eta heta
    calc
      _ ≤ (∑ i ∈ rows, ∑ j ∈ rows,
          p53AnalyticPairBudget S alpha (Real.exp xi) (Real.exp upsilon) i j) +
        (rows.card : ℝ) *
      ...
      _ = (∑ i ∈ rows, ∑ j ∈ rows,
        ...
        (rows.card : ℝ) *
          ((12 * (upsilon - xi) +
              48 * Real.exp 1 * integerExponentialMass) *
            (harmonic R : ℝ)) := by rw [Real.log_exp, Real.log_exp]
...
end MAPJutilaP53SharpIntegratedPairSectorAdapter

#print axioms MAPJutilaP53SharpIntegratedPairSectorAdapter.integratedCorrelationEstimateAt_of_sharpPairSectorEnvelopes
