...
# The outer separator must not consume lean4export's inner target separator.
assert_passthrough "explicit separators survive at both argument levels" \
  '--ro
/workspace
--
lean4export
Solution
--
AllCenterMAP.map_two_fifteenths' \
  --ro /workspace -- lean4export Solution -- AllCenterMAP.map_two_fifteenths

run_wrapper --
if [ "$wrapper_status" -ne 2 ] || [ -s "$PALOMAR_TEST_ARGV" ]; then
  report_failure "an explicit separator without a command must fail closed"
fi
