...
/-!
# Unequal-scale selected-range Lemma 9.2 endpoint

The second Poisson scale `M₂` is the positive dyadic integer `M`, while the
affine first-Poisson scale `M₃` remains independent.  This is the literal
three-factor source interface; no comparison `M₂=M₃` is imposed.
-/

/-- Selected-range second-Poisson endpoint with a variable ell-detector radius. The exact scalar budget remains visible; no final large-value estimate is assumed. -/
theorem sigmaIIFinite_selectedPositiveDyadic_variableEllRadius_unequalScale_le
    ...
    (q : ℕ) {M3 R Ctail : ℝ} (hR : 0 < R) (hRone : 1 ≤ R) (hM3pos : 0 < M3)
    (hT : 1 ≤ T) (hF0 : 0 ≤ F) (_hF : F ≤ T)
    (_hMhi : (M : ℝ) ≤ T ^ 4) (hCtau : 0 ≤ Ctau)
    (_hCtail : 0 ≤ Ctail)
    (hbudget :
      ((T / (M : ℝ)) *
    ...
    sigmaIIFinite (sourceBumpEllRange M T R) mRange
          ...
          (M : ℝ) T M3 Ctau ≤
        ((2 * Ctau * (R * sourceBumpFourierConstant 1 zero_lt_one 0)) * (2 : ℝ) ^ 2 * (M : ℝ)) *
          Real.sqrt ((∫ u : ℝ, f u ^ 2) *
            sourceAffineJ
  ...
  have hM3ne : M3 ≠ 0 := hM3pos.ne'
  obtain ⟨hKdec, hKsup, hdecay2, hdecay, hFourierSup⟩ :=
    sourceBump_scaled_fourier_package hR hRone q
  ...
  have hsupport : ∀ ell : ℤ,
      ell ∉ sourceBumpEllRange M T R →
        sourceBump R hR ((M : ℝ) * (ell : ℝ) / T) = 0 :=
    sourceBump_support_on_sourceBumpEllRange hM hTpos hR
  have hret : ∀ m2 ∈ mRange, ∀ m2' ∈ mRange,
      Integrable (sigmaIIZPairRetainedKernel
        (fun x => sourceBump R hR x) f
        (M : ℝ) T M3 Ctau (T ^ delta) m2 m2')
        (volume.prod volume) := by
    intro m2 hm2 m2' hm2'
    exact integrable_sigmaIIZPairRetainedKernel_of_budget
      (sourceBumpEllRange M T R) (fun x => sourceBump R hR x) f
      hf.integrable (sourceBump_complex_hasCompactSupport R hR)
      (sourceBump_complex_contDiff R hR) q hKdec hMreal hTpos
      hBpos hY hM3ne hCtau hdecay2 hdecay hbudget m2 m2'
  ...
  have htail : ∀ m2 ∈ mRange, ∀ m2' ∈ mRange,
    ...
    exact integrable_sigmaIIZPairTailKernel_of_budget
      (M3 := M3) (fun x => sourceBump R hR x) f
      hf.integrable (sourceBump_complex_hasCompactSupport R hR)
      (sourceBump_complex_contDiff R hR) q hKdec hMreal hTpos
  ...
  have hsmall := sigmaIIFinite_le_canonicalAffineJ_sqrt_add_time_neg100
    f (fun x => sourceBump R hR x) psi hf.integrable
    (sourceBumpEllRange M T R) mRange jRange
    (sourceBump_complex_hasCompactSupport R hR)
    (sourceBump_complex_contDiff R hR) q hKdec hKsup
    hMreal hTpos hBpos hY hM3ne hCtau (show 0 ≤ (2 : ℝ) by norm_num)
    ...
    (fun m2 hm2 m2' hm2' =>
      hp.pairMajorantIntegrable
        ((2 * Ctau * (R * sourceBumpFourierConstant 1 zero_lt_one 0)) / (M : ℝ))
...
#print axioms GuthMaynardJIteration.sigmaIIFinite_selectedPositiveDyadic_variableEllRadius_unequalScale_le
