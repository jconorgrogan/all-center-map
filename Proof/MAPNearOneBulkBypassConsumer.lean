...
/-! Weighted-zero consumer for fixed-strip bulk density plus only the Jutila
collar. The epsilon-fine mesh, multiplicities and regular zero masks are the
existing source-faithful ones. -/
namespace MAPNearOneBulkBypassConsumer
open Filter Set
open scoped BigOperators
open DirichletZeros MAPGuthMaynard MAPAPZeroDensityCert
open CGLMeshFormalization MAPCGLNearFineMesh MAPCGLNearFineMeshSourceAdapter
open MAPRelativeNearOneMesh27 MAPJutilaCollarMeshCutoff
open MAPNearOneBulkBypassInterface
noncomputable section

/-- Conductor conversion is explicit: `log X >= 4` and `tau >= 1/2` imply
`(log T)^2 >= log X`, so the source is invoked with exponent `2K`. -/
theorem eventually_fixedPrimitiveNearDensity_of_bulk
    (hbulk : FixedPrimitiveBulkPolylogDensity)
    (K epsilon eta : ℝ) (hK : 0 < K) (hepsilon : 0 < epsilon)
    (hepsilonCap : epsilon ≤ 1 / 10) (heta : 0 < eta) :
    ∃ C : ℝ, 0 < C ∧
      ∀ᶠ X : ℝ in atTop,
        ∀ (Q : ℕ), (Q : ℝ) ≤ Real.rpow (Real.log X) K →
  ...
  obtain ⟨C, T₀, hC, hT₀, hsource⟩ := hbulk (2 * K) eta (by positivity) heta
  have htau : 0 < tau epsilon := tau_pos hepsilonCap
  have htauHalf : 1 / 2 ≤ tau epsilon := by unfold tau; linarith
  have hheight : ∀ᶠ X : ℝ in atTop, T₀ ≤ Real.rpow X (tau epsilon) :=
    (tendsto_rpow_atTop htau).eventually (eventually_ge_atTop T₀)
  refine ⟨C, hC, ?_⟩
  filter_upwards [hheight, eventually_ge_atTop (Real.exp 4)] with X hheightX hXlarge
  intro Q hQ sigma hsigmaLow hsigmaHigh r _inst chi hprim hrQ
  have hXpos : 0 < X := (Real.exp_pos 4).trans_le hXlarge
  have hlog : 4 ≤ Real.log X := by
    rw [← Real.log_exp 4]
    exact Real.log_le_log (Real.exp_pos 4) hXlarge
  have hlog0 : 0 ≤ Real.log X := by linarith
  let T := Real.rpow X (tau epsilon)
  have hlogT : Real.log T = tau epsilon * Real.log X := Real.log_rpow hXpos _
  have hhalf : Real.log X / 2 ≤ Real.log T := by
    rw [hlogT]
    nlinarith [mul_le_mul_of_nonneg_right htauHalf hlog0]
  have hlogT0 : 0 ≤ Real.log T := by linarith
  have hsq : Real.log X ≤ (Real.log T) ^ 2 := by nlinarith
  have hpower : Real.rpow (Real.log X) K ≤ Real.rpow (Real.log T) (2 * K) := by
    calc
      _ ≤ Real.rpow ((Real.log T) ^ 2) K := Real.rpow_le_rpow hlog0 hsq hK.le
      _ = _ := by
        simpa using (Real.rpow_mul hlogT0 (2 : ℝ) K).symm
  have hQsource : (Q : ℝ) ≤ Real.rpow (Real.log T) (2 * K) := hQ.trans hpower
  have hcap : sigma ≤ 559 / 560 := by
    have hd := collarDistance_lower
    unfold relativePoint at hsigmaHigh
    linarith
  exact hsource T Q sigma hheightX hQsource hsigmaLow hcap r chi hprim hrQ
...
theorem regularNear_logSaving_of_bulk_collar_and_high_gap
    (hbulk : FixedPrimitiveBulkPolylogDensity)
...
/-- The exact weighted-zero endpoint needs only compact density, the proved
fixed-strip bulk output, Jutila on the collar, and the regular high gap. -/
theorem apWeightedZeroMassLogSaving_of_bulk_collar_and_high_gap
  ...
  MAPAPExceptionalUnconditional.apWeightedZeroMassLogSaving_of_compact_regular
    hCompact (regularNear_logSaving_of_bulk_collar_and_high_gap hbulk hJutila hHighGap)

#print axioms eventually_fixedPrimitiveNearDensity_of_bulk
#print axioms regularNear_logSaving_of_bulk_collar_and_high_gap
#print axioms apWeightedZeroMassLogSaving_of_bulk_collar_and_high_gap
end
end MAPNearOneBulkBypassConsumer
